...
def log(sql, args=()):
    logging.info(f"SQL: {sql} with args {args}")


@asyncio.coroutine
def create_pool(loop, **kw):
    logging.info("create data connection pool...")
    global __pool
...
class ModelMetaclass(type):
    def __new__(mcs, name, bases, attrs):
        ...
        temp = ", " + table_name + "."
        attrs["__select__"] = "select %s.`%s`, %s%s from `%s`" % (
            table_name,
            primary_key,
            table_name + ".",
            temp.join(escaped_fields),
            table_name,
        )
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    @classmethod
    async def find_all(cls, where=None, inner_join=None, args=None, **kw):
        """
    ...
    @classmethod
    async def find_number(cls, select_field, where=None, args=None, inner_join=None):
